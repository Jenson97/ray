...
class LLMRouter:
    ...
    async def __init__(self, server: DeploymentHandle):
        ...
        self._handle._init()
        # Tokenize pre-routing so token IDs can be passed into choose_replica
        # for KV-aware request routers to score against.
    ...
    async def route(self, request: Request):
        ...
        # Tokenize before routing so a KV-aware request router can score
        # replicas against the prompt token IDs. Best-effort: None on failure.
        request_token_ids = await self._route_tokenizer.tokenize(body, body_truncated)
        try:
